...
//Defines for library extension
#define SCALE 2			//Accelerometer has scale of +-2g
#define XADJUST 0	//X-axis adjust for real value
#define YADJUST -0.05	//Y-Axis adjust for real value
#define ZADJUST -0.05	//Z-axis adjust for real value
#define ALPHA 0.5
...
LSM303::LSM303(void)
{
  ...
  */
  ...
  filteredAcceleration.x = 0;
  filteredAcceleration.y = 0;
  filteredAcceleration.z = 0;
...
void LSM303::readAcc(void)
{
  ...
  //Library Extension. Get actual value in g for raw accelerometer values and then filter
  ra.x = (a.x /pow (2,15)* SCALE) + XADJUST;
  ra.y = a.y /pow (2,15)* SCALE + YADJUST;
  ra.z = a.z /pow (2,15)* SCALE + ZADJUST;
...
float LSM303::getPitch(void)
{
	pitch = (atan2(filteredAcceleration.x, sqrt(filteredAcceleration.y*filteredAcceleration.y + filteredAcceleration.z*filteredAcceleration.z))*180.0)/M_PI;
    return pitch;
}

float LSM303::getRoll(void)
{
    roll  = (atan2(-filteredAcceleration.y, filteredAcceleration.z)*180.0)/M_PI;
	return roll;
}

void LSM303::lpFilter(void)
{
	filteredAcceleration.x = (ra.x * ALPHA) + (filteredAcceleration.x * (1-ALPHA));
	filteredAcceleration.y = (ra.y * ALPHA) + (filteredAcceleration.y * (1-ALPHA));
	filteredAcceleration.z = (ra.z * ALPHA) + (filteredAcceleration.z * (1-ALPHA));
}
